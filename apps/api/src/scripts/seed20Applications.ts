 ...
 *   - 10 evaluated           (passed evaluation, WSM scored, awaiting TOPSIS)
 *   - 5  verification_pending  (TOPSIS scored, assigned to verifier, not yet visited)
 *   - 3  approved          (full pipeline complete, approved)
 *   - 1  rejected          (post-verification, rejected)
 *   - 1  anomaly_flagged   (evaluation fraud detection fired)
 *
 * Run from apps/api/:
...
interface StudentProfile {
  ...
  // Seeder control fields ('evaluated' = WSM scored, awaiting TOPSIS)
  _stage: 'evaluated' | 'verification_pending' | 'approved' | 'rejected' | 'anomaly_flagged'
...
const STUDENTS: StudentProfile[] = [
  ...
  // 
  //  EVALUATED (1-10): WSM scored, awaiting admin TOPSIS trigger
  // 
...
async function main() {
  ...
  console.log('  10 evaluated  5 verification_pending  3 approved  1 rejected  1 anomaly')
  console.log('\n')
  ...
  for (const s of STUDENTS) {
    ...
    const appStatus = isAnomaly ? 'anomaly_flagged' : s._stage === 'evaluated' ? 'evaluated' : s._stage === 'verification_pending' ? 'verification_pending' : s._stage === 'approved' ? 'approved' : 'rejected'
