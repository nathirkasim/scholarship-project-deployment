/**
 * 15-Application Seeder
 * - Wipes all student data
 * - Creates 15 fresh student profiles + applications
 * - Runs Phase 1 (anomaly) + Phase 3 (scoring) + TOPSIS
 * - Top 5 by composite score → verification_pending (assigned to verifier)
 * - Remaining 10 → scored
 */
...
// ── 15 Profiles ────────────────────────────────────────────────────────────────
// Profiles 1–5  : High need + merit → will reach verification_pending
// Profiles 6–15 : Lower scores      → stay at scored

const STUDENTS: StudentProfile[] = [

  // 1 — Priya Sharma: SC orphan, BPL, kuccha, no utilities, 85%
  {
  ...
  // 2 — Murugan Selvam: ST tribal AAY, 7-member family, chronic illness, 73%
  {
  ...
  // 3 — Lakshmi Devi: SC, differently-abled, single parent, BPL, 78%
  {
  ...
  // 4 — Anbarasan Raj: ST tribal orphan, first-gen, 82%
  {
  ...
  // 6 — Valli Kannan: SC, widow pension, BPL, single parent, 80%
  {
  ...
  // 7 — Selvi Arumugam: ST tribal, AAY, 7-member, 74%
  {
  ...
  // 8 — Ramesh Babu: SC, BPL orphan, semi-pucca, income 1.5L, 79%
  {
  ...
  // 9 — Pushpa Nair: OBC, rural, single parent widow, BPL, 77%
  {
  ...
  // 10 — Durai Pandian: ST tribal, AAY+MGNREGA, income 60K, 7-member, 69%
  {
  ...
  // 13 — Vijay Ramachandran: General, urban, income 7L, flat, 80%
  {
  ...
  // 14 — Anitha Mohan: OBC, urban, income 4L, pucca rented, 70%
  {
...
async function main() {
  console.log('\n' + hr())
  console.log('  15-APPLICATION SEEDER  |  top 5 → verification_pending')
  console.log(hr())

  // Step 1: Wipe
  console.log('\n  [1/5] Wiping existing student data...')
  ...
  if (!program) { console.error('\n  No active program. Run: npm run db:seed\n'); process.exit(1) }
  ...
  // Step 3: Create profiles + applications
  console.log('\n  [2/5] Creating 15 users, profiles, and applications...')
  ...
  // Step 4: Phase 1 anomaly + scoring
  console.log('\n  [3/5] Phase 1 anomaly check + rule engine scoring...')
  let anomalyRejected = 0
  for (const appId of applicationIds) {
    try { await runAnomalyPreFilter(appId) } catch { /* ML offline — rules still run */ }
    const a = await prisma.application.findUniqueOrThrow({ where: { id: appId } })
    if (a.anomaly_flag) anomalyRejected++
    process.stdout.write('.')
  }
  console.log(`\n   Anomaly rejected: ${anomalyRejected}`)

  // Step 5: TOPSIS ranking
  console.log('\n  [4/5] TOPSIS batch ranking...')
  await runTOPSIS(program.id)

  // Step 6: Top 5 → verification_pending
  console.log('\n  [5/5] Setting top 5 to verification_pending...')
  const scoredApps = await prisma.application.findMany({
    where:   { program_id: program.id, status: 'scored' },
    orderBy: { composite_score: 'desc' },
    select:  { id: true, composite_score: true, composite_rank: true, user: { select: { full_name: true } } },
  })

  const top5 = scoredApps.slice(0, 5)
  ...
  for (let i = 0; i < top5.length; i++) {
    ...
    await prisma.verificationAssignment.upsert({
      ...
      update: { verifier_id: verifier.id, assigned_at: new Date(), status: 'pending', verification_priority: i + 1 },
      create: { application_id: app.id, verifier_id: verifier.id, assigned_at: new Date(), status: 'pending', verification_priority: i + 1 },
    })
    console.log(`   #${i + 1} ${app.user.full_name.padEnd(24)} TOPSIS: ${Number(app.composite_score).toFixed(2)} → verification_pending`)
  }
  ...
  console.log(hr())
  console.log('  #   Name                      Merit   Need   TOPSIS  Status')
  console.log('  ' + hr('-', 70))

  for (let i = 0; i < all.length; i++) {
    const a     = all[i]!
    const name  = a.user.full_name.padEnd(24)
    const merit = a.merit_score     != null ? Number(a.merit_score).toFixed(1).padStart(5)     : '    -'
    const need  = a.rule_need_score != null ? Number(a.rule_need_score).toFixed(1).padStart(5) : '    -'
    const top   = a.composite_score != null ? Number(a.composite_score).toFixed(2).padStart(7) : '      -'
    const flag  = a.anomaly_flag ? ' ⚑' : ''
    console.log(`  ${String(i + 1).padStart(2)}  ${name} ${merit}  ${need}  ${top}  ${a.status}${flag}`)
  }

  const vPending = all.filter(a => a.status === 'verification_pending').length
  const scored   = all.filter(a => a.status === 'scored').length
  const rejected = all.filter(a => a.status === 'rejected').length

  console.log('\n  verification_pending : ' + vPending)
  console.log('  scored               : ' + scored)
  console.log('  rejected (anomaly)   : ' + rejected)
