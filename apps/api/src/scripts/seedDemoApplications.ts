...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < STUDENTS.length; i++) {
    ...
    // Phase 3 scoring is done inside runEvaluation (called above as runAnomalyPreFilter).
    // Just display the stored scores from the updated record.
    if (updated.status === 'evaluated') {
      console.log('\n   Phase 3  Rule Engine Scoring ...')
      console.log(pad('  Status:', updated.status))
      console.log(pad('  Merit score   (Domain A):', Number(updated.merit_score ?? 0).toFixed(2) + ' / 100'))
      console.log(pad('  Need score    (BF):', Number(updated.rule_need_score ?? 0).toFixed(2) + ' / 100'))
      console.log(pad('  Integrity adj (Domain G):', Number(updated.integrity_adj ?? 0).toFixed(2)))
      console.log(pad('  WSM Provisional:', Number(updated.wsm_provisional ?? 0).toFixed(2) + ' / 100'))
      console.log(pad('  Note:', 'composite_score set by TOPSIS (admin-triggered)'))
    }
    ...
    results.push({
      ...
      composite:   updated.wsm_provisional ? Number(updated.wsm_provisional) : null,
    })
  }
  ...
  console.log(hr(''))
  console.log('  #  Applicant               Status                Merit   Need   Integrity  WSM Prov.')
  console.log('  ' + hr('-', 87))
  ...
  console.log(`
  Notes:
   WSM Prov. = 0.35  Merit  +  0.65  Need  +  Integrity_adj (composite_score set by TOPSIS)
