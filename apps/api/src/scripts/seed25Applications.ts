...
import { runScoringEngine, runTOPSIS, applyVerificationMultiplier } from '../services/scoring/ruleEngine'
...
async function main() {
  ...
  console.log('\n  [1/8] Wiping existing applications and student profiles...')
  ...
  console.log('\n  [2/8] Creating 25 users, profiles, and applications...')
  ...
  console.log('\n  [3/8] Phase 1  Anomaly pre-filter (G-01..G-07 + Isolation Forest)...')
  ...
  //  Step 5: Phase 3  Rule engine scoring 
  console.log('\n  [4/8] Phase 3  Rule engine scoring (Domains AF + integrity)...')
  let scored = 0
  for (const appId of applicationIds) {
    const app = await prisma.application.findUniqueOrThrow({ where: { id: appId } })
    if (app.status !== 'shortlisted') continue
    try { await runScoringEngine(appId); scored++ } catch (e) { console.error(`   Scoring failed for ${appId}:`, (e as Error).message) }
    process.stdout.write('.')
  }
  console.log(`\n   Phase 3 complete  ${scored} applications scored`)

  //  Step 6: TOPSIS batch ranking 
  console.log('\n  [5/8] TOPSIS batch ranking (cohort normalisation)...')
  ...
  console.log('\n  [6/8] Simulating field verification (match scores)...')
  ...
  console.log('\n  [7/8] Applying verification multipliers (I-02/I-03/I-04)...')
  ...
  console.log('\n  [8/8] Finalising decisions (top 10  approved, 11-25  rejected)...')
