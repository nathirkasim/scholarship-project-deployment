...
const STATUS_META: Record<string, { label: string; dot: string; badge: string }> = {
  ...
  anomaly_checking:      { label: 'Fraud Check',         dot: 'bg-amber-400',   badge: 'bg-amber-50 text-amber-700' },
  anomaly_flagged:       { label: 'Flagged',             dot: 'bg-red-500',     badge: 'bg-red-50 text-red-700' },
  scoring_pending:       { label: 'Pending Score',       dot: 'bg-indigo-400',  badge: 'bg-indigo-50 text-indigo-700' },
  shortlisted:           { label: 'Shortlisted',         dot: 'bg-cyan-500',    badge: 'bg-cyan-50 text-cyan-700' },
...
export default function AdminApplicationsPage() {
  ...
  async function trigger(action: 'scoring' | 'verification') {
    setTriggering(action)
    const url = action === 'scoring' ? '/api/proxy/officer/trigger-scoring' : '/api/proxy/officer/trigger-verification'
    const res = await fetch(url, { method: 'POST', credentials: 'include' })
    const d = await res.json()
    res.ok ? showToast(d.message) : showToast('Action failed', false)
    setTriggering(null)
  }
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4 flex-shrink-0">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex gap-2">
            <button onClick={() => trigger('scoring')} disabled={triggering !== null}
              ...
              {triggering === 'scoring' ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Zap className="w-4 h-4" />}
              Run Scoring
            </button>
              ...
              {filtered.length === 0 ? (
                ...
                return (
                  <tr key={app.id} className="bg-white hover:bg-blue-50/40 transition-colors group">
                    ...
                    <td className={`px-4 py-3.5 font-semibold tabular-nums ${Number(app.integrity_adj) < 0 ? 'text-red-500' : 'text-slate-400'}`}>
                      {app.integrity_adj != null ? Number(app.integrity_adj).toFixed(1) : ''}
                    </td>
                    <td className="px-4 py-3.5">
